...
hw_power_PowerSource_t hw_power_GetVddaPowerSource(void) {
    ...
    if (HW_POWER_VDDACTRL.B.DISABLE_FET) {
        ...
        // Make sure the LinReg offset is correct for this source.
        //----------------------------------------------------------------------
        if (hw_power_GetVddaLinRegOffset() ==
            HW_POWER_LINREG_OFFSET_LINREG_MODE) {
            return HW_POWER_LINREG_DCDC_OFF;
        }
    }
...
void hw_power_PrintExternalLdoStatusCallback(uint32_t Status) {
    if (Status) {
        ...
        tss_logtext_Print(LOGTEXT_VERBOSITY_1,
                          "HW_PWR: External LDO - OFF\r\n");
    }
}

void hw_power_PrintExternalLdoStatus(uint32_t Status) {
    os_dpc_Send(OS_DPC_HIGH_LEVEL_DPC, &hw_power_PrintExternalLdoStatusCallback,
                Status, TX_NO_WAIT);
}
#endif
...
////////////////////////////////////////////////////////////////////////////////
bool hw_power_GetDcdc4p2Brownout(void) { return HW_POWER_STS.B.DCDC_4P2_BO; }
...
void hw_power_Enable4p2DcdcInput(bool bEnable) {
    ...
    if (bEnable) {
        ...
        HW_POWER_5VCTRL_SET(
            BF_POWER_5VCTRL_VBUSVALID_TRSH(orig_vbusvalid_threshold));

        /* give a little time after writing new vbusvalid threshold for
         * glitch/noise to exceed vbusvalid_threshold.  Glitch/noise should be
         * gone but this is an attempt to handle it in case it is not.
         */

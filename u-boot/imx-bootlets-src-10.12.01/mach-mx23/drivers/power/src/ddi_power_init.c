...
//!
//! \param[in]  eLradcDelayTrigger    LRADC delay trigger for battery
//! measurement
//!
...
RtStatus_t ddi_power_Init(ddi_power_InitValues_t *pInitValues) {
    ...
    if (Is5vDetectionInitialized() == false) {
        Status = ddi_power_Configure5vDetection(
            pInitValues->eDcdc5vDetection, pInitValues->e5vDetection,
    ...
    if (IsBatteryMonitorInitialized() == false) {
        ...
        if ((Status = hw_power_InitBatteryMonitor(
                 eDelayTrigger, u32SampleInterval)) != SUCCESS)
            return Status;
    }
...
bool IsHwPowerInitialized(void) {
    ...
    if (HW_POWER_LOOPCTRL.B.EN_CM_HYST == 0 ||
        HW_POWER_LOOPCTRL.B.EN_DF_HYST == 0)
        return false;
...
bool Is5vIrqInitialized(void) { return false; }

...
void hw_lradc_EnableTempSensorCurrent(hw_lradc_TempSensor_t eSensor,
    ...
    switch (eSensor) {
    case TEMP_SENSOR0:
        if (bValue) { // Enable the sensor current source
            BF_SET(LRADC_CTRL2, TEMP_SENSOR_IENABLE0);
        } else { // Disable the sensor current source
...
////////////////////////////////////////////////////////////////////////////////
uint16_t
hw_lradc_MeasureBatteryTemperature(hw_lradc_Channel_t BattTempChannel) {
    ...
    //--------------------------------------------------------------------------
    {
        ...
        //----------------------------------------------------------------------
        {
            u16Temp = ((u32HighCurrentReading - u32LowCurrentReading) *
                       TEMP_SENSOR_CONVERSION_CONSTANT) /
                      1000;
        }
    }

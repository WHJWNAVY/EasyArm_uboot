 ...
 * This includes configurations with and without a supercap on the BATTERY pin
 * as shown in the i.MX28 reference schematic (applies to i.MX233 as well).
 *
 * NOTE: If both a DCDC_BATT and VDD5V source are present, this is not meant to
 * select which one to use.  This is only for hardware configurations this no
 * VDD5V power source.
...
bool IsVbusValid(void) { return hw_power_GetVbusValid(); }

bool IsVdd5vGtVddio(void) { return hw_power_GetVdd5vGtVddio(); }
...
#define SSP0_PIN_DRIVE_12mA 0x2
    // For EMI 200MHz,must enable SSP0 pin drive to 12mA,or the SD boot will
    // fail.
    HW_PINCTRL_DRIVE8_CLR(
        BM_PINCTRL_DRIVE8_BANK2_PIN07_MA | BM_PINCTRL_DRIVE8_BANK2_PIN06_MA |
        BM_PINCTRL_DRIVE8_BANK2_PIN05_MA | BM_PINCTRL_DRIVE8_BANK2_PIN04_MA |
        BM_PINCTRL_DRIVE8_BANK2_PIN03_MA | BM_PINCTRL_DRIVE8_BANK2_PIN02_MA |
        BM_PINCTRL_DRIVE8_BANK2_PIN01_MA | BM_PINCTRL_DRIVE8_BANK2_PIN00_MA);

    HW_PINCTRL_DRIVE9_CLR(BM_PINCTRL_DRIVE9_BANK2_PIN10_MA |
    ...
    HW_POWER_CTRL_CLR(BM_POWER_CTRL_VDDD_BO_IRQ | BM_POWER_CTRL_VDDA_BO_IRQ |
                      BM_POWER_CTRL_VDDIO_BO_IRQ |
                      BM_POWER_CTRL_VDD5V_DROOP_IRQ |
                      BM_POWER_CTRL_VBUSVALID_IRQ | BM_POWER_CTRL_BATT_BO_IRQ |
...
void PowerPrep_SetupBattDetect(void) {
    ...
    hw_lradc_EnableBatteryMeasurement(
        hw_lradc_AcquireBatteryMonitorDelayTrigger(), 100);
}
...
void PowerPrep_Enable4p2(void) {
    ...
    /* Errata 5837:  Setting the ENABLE_DCDC bit in the HW_POWER_DCDC4P2 or
       ...
       When the ENABLE_DCDC bit in HW_POWER_DCDC4P2 is set, a glitch is
       propagated through the brownout comparators. If the glitch is
       sufficiently large, it can cause a false brownout detection. The VDDD,
       VDDA, VDDIO, and VBUSVALID comparators are all susceptible to the glitch.
       Below codes follow the workaround sequence of Errata 5837 to avoid the
       shutdown caused by false brownout.
    */
    ...
    /* before enabling vdda fet, give some small delay to allow DCDC to handle
     * new load if vddd fet is getting enabled */
    hw_digctl_MicrosecondWait(20);
    HW_POWER_VDDACTRL_WR(vddactrl);
    /* before enabling vddio fet, give some small delay to allow DCDC to handle
     * new load if vdda fet is getting enabled */
    hw_digctl_MicrosecondWait(20);
...
void PowerPrep_Init4p2Regulator(void) {
    ...
    while (i <
           hw_power_ConvertCurrentToSetting(BOOTUP_CHARGE_4P2_CURRENT_LIMIT)) {
        i++;
...
int PowerPrep_Handle5vConflict(void) {
    ...
    // Need to check the VDDIO brownout level, so initialize it to maximum
    // offset.
...
bool PowerPrep_IsBatteryReady(void) {
    ...
    if (hw_power_GetBatteryVoltage() >= MINIMUM_SAFE_BOOTING_BATTERY_VOLTAGE_MV)
        return true;
    else
        return false;
}
...
void PowerPrep_CPUClock2PLL(void) {
    ...
    PowerPrep_Delay(100);
    /*HW_CLKCTRL_CPU_WR((HW_CLKCTRL_CPU_RD() & (~BM_CLKCTRL_CPU_DIV_CPU)) |
     * 0x14);*/

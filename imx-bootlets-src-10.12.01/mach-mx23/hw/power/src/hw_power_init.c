...
RtStatus_t hw_power_Init(void) {
    ...
    if (bMasterFlagsInitialized == false) {
        ...
        bEnableMaster4p2 =
            HW_POWER_DCDC4P2.B.ENABLE_4P2 && !HW_POWER_5VCTRL.B.PWD_CHARGE_4P2;
...
RtStatus_t hw_power_InitBatteryMonitor(hw_lradc_DelayTrigger_t eTrigger,
    ...
    //----------------------------------------------------------------------
    rtn = hw_lradc_EnableBatteryMeasurement(eTrigger, u32SamplingInterval);
...
RtStatus_t hw_power_InitPowerSupplies(void) {
    ...
    if (hw_power_Get5vPresentFlag()) {
        ...
        if ((Status = hw_power_SetVdddPowerSource(
                 HW_POWER_LINREG_DCDC_READY)) != SUCCESS)
            return Status;
        if ((Status = hw_power_SetVddaPowerSource(
                 HW_POWER_LINREG_DCDC_READY)) != SUCCESS)
            return Status;
        if ((Status = hw_power_SetVddioPowerSource(
                 HW_POWER_LINREG_DCDC_READY)) != SUCCESS)
            return Status;
    } else {

        if ((Status = hw_power_SetVdddPowerSource(
                 HW_POWER_DCDC_LINREG_READY)) != SUCCESS)
            return Status;
        if ((Status = hw_power_SetVddaPowerSource(
                 HW_POWER_DCDC_LINREG_READY)) != SUCCESS)
            return Status;
